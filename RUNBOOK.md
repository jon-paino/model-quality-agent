...
| `NoClassDefFoundError: Could not initialize class ai.onnxruntime.OrtEnvironment` at scorer start (flow dies; source still reads, nothing scores) | The Striim server JVM is x86_64 on an Apple-Silicon Mac (Rosetta — often Migration-Assistant residue). The bundled ONNX Runtime 1.26 ships **no mac-Intel natives** (`osx-aarch64`/`linux-x64`/`linux-aarch64`/`win-x64` only). Confirm with `file "$(ps -o command= -p <server pid> \| awk '{print $1}')"`; fix by running Striim on a **native arm64 JDK** of the same Java major version, then teardown + re-paste + re-feed under a fresh filename. The first-ever occurrence in the server log shows the underlying `UnsatisfiedLinkError`; later deploys only show the `NoClassDefFoundError` |
